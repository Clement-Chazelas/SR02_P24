...
void fonct_gestionnaire() {
    //fin du processus gestionnaire de tache
    exit(0);
}
...
void fonct_pere() {
    ...
    kill(pid, SIGUSR1); // Envoie un signal au processus enfant (gestionnaire) pour qu'il arrête la tâche
    close(fd);
    exit(0);
}

void fonct_tache() {
   // Fin d'une tache qui est trop longue
    exit(0);
}
...
void exec_tache(tache* tache) {
    ...
    if (tache_pid < 0) {
        ...
        if (timer_pid == 0) {
            ...
            if (kill(getppid(), SIGUSR2) == 0) { //kill renvoi 0 si le processus existe encore (délai dépassé)
...
int main() {
    pid = fork();
    if (pid == 0) {
       // Mise en place du gestionnaire de tache
        ...
        tache readtache = {-1,-1};
        while (1) {
            if (read(fd, &readtache, sizeof(tache)) < 0) { // lecture du pipe
                perror("Error reading pipe");
                exit(1);
            }
            if (readtache.duree_exec != -1) { //durée = -1 si aucune nouvelle tache n'est lu
